...
from qwen_ocr import ConfigurationError, OCRConfig


def test_config_from_env_with_api_key():
    """Test creating config from environment with API key."""
    os.environ["QWEN_OCR_API_KEY"] = "test_key"
    os.environ["QWEN_OCR_BASE_URL"] = "https://api.test.com/v1/chat/completions"
    config = OCRConfig.from_env()
    assert config.api_key == "test_key"
    assert config.base_url == "https://api.test.com/v1/chat/completions"
    assert config.dpi == 200
    del os.environ["QWEN_OCR_API_KEY"]
    del os.environ["QWEN_OCR_BASE_URL"]
