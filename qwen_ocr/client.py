"""
Core OCR client for Qwen-VL-OCR SDK.

This module provides the main client for interacting with the Qwen-VL-OCR API.
"""
...
class QwenOCR:
    """
    Client for Qwen-VL-OCR API.

    This client provides both synchronous and asynchronous methods for
    document OCR processing using the Qwen-VL-OCR API.
    ...
    def __init__(
        ...
        **kwargs: str,
        ...
        logger.info(
            f"Initialized QwenOCR client with model: {self.config.model_name}"
        )
    ...
    async def _make_api_request_async(
        ...
        """
        Make async API request to Qwen-VL-OCR.
    ...
    def _make_api_request_sync(self, image_b64: str, prompt: str) -> Dict[str, Any]:
        """
        Make synchronous API request to Qwen-VL-OCR.
    ...
    async def parse_async(
        self,
        file_path: Union[str, Path],
        mode: Union[str, OCRMode] = OCRMode.FAST,
        dpi: Optional[int] = None,
        pages: Optional[Union[int, List[int]]] = None,
    ) -> str:
        """
        ...
        Intelligent Fallback:
            ...
            (default: "standard"). This per-page approach ensures optimal
            results without reprocessing all pages when only some need
            better quality.

        Args:
            file_path: Path to PDF or image file.
            mode: OCR mode ("fast", "standard", "detailed" or enum).
